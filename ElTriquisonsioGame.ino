...
void drawStartScreen()
{
  ...
  tft.setTextSize(3);
  tft.print("EL TRIQUISONSIO");
  
  
  tft.setCursor(30,50);
  tft.setTextColor(GREEN);
  tft.setTextSize(2);
  tft.print("Sebas, Jose y Cristian");
...
void drawHorizontalLine(int y)
{
  ...
  for(i=0;i<7;i++)
  {
    tft.drawLine(60,y+i,270,y+i,RED);                                   //
  }
}
...
void drawVerticalLine(int x)
{
  ...
  for(i=0;i<7;i++)
  {
    tft.drawLine(x+i,20,x+i,220,RED);                                    //
  }
}
...
{
  drawBitmap(x,y,circle,65,65,BLUE);                        //
}
